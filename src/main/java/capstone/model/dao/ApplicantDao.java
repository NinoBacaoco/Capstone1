...
public interface ApplicantDao extends JpaRepository<ApplicantEntity, Integer> {
	...
	// + " CASE "
	// + " WHEN a.status > 0 "
	// + " THEN (SELECT DISTINCT CONCAT(u2.first_name, ' ', u2.last_name) AS
	// full_name "
	// + " JOIN m_user_information u2 ON u2.id_pk = r2.created_by AND u2.delete_flg
	// = false "
	// + " WHERE r2.applicant_id_pk = a.id_pk"
	// + " AND r2.id_pk = (SELECT MAX(id_pk) FROM t_accepted_applicant WHERE
	// applicant_id_pk = a.id_pk)"
	// + " ) "
	// + " ELSE '' "
	// + " END "
	// + " ) AS accepted_by, "
	...
	// + " JOIN m_user_information u2 ON u2.id_pk = e2.created_by AND u2.delete_flg
	// = false "
	// + " WHERE e2.applicant_id_pk = a.id_pk"
	// + " AND e2.id_pk = (SELECT MAX(id_pk) FROM t_evaluated_applicant WHERE
	// applicant_id_pk = a.id_pk)"
	// + " ) "
	// + " ELSE '' "
	// + " END"
	...
	// + "AND e.id_pk = (SELECT MAX(id_pk) FROM t_evaluated_applicant WHERE
	// applicant_id_pk = a.id_pk) "
	...
	public void updateApplicant(@Param("agreeFlg") Boolean agreeFlg,
			...
			@Param("idPk") int idPk) throws DataAccessException;
	...
	public List<Object[]> getHistoryApplicantDetailsByIdPkRaw(int applicantIdPk, int projectIdPk)
